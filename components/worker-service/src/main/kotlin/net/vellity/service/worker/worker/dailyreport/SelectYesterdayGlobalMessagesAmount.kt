...
class SelectYesterdayGlobalMessagesAmount(private val context: Context = Context.ALL) : SelectQuery<Int> {
  override fun query(): String = if (context == Context.ALL) {
    "select count(*) as amount from usercache_users_messages where date(createdAt) = date(now() - interval 1 day)"
  } else {
    "select count(*) as amount from usercache_users_messages where date(createdAt) = date(now() - interval 1 day) and context = ?"
  }
